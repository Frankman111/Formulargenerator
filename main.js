...
let java; // Variable für erworbene Grundkenntnisse
let sap;
let nw;
let pm;
let web;
...
// Abfrage des Wissenstandes
    ...
    // Evaluierung der abgelegten Kompetenzen

    function finComp(){
        for( let i = 0; i<comp.length; i++){
            if (comp[i].checked){
                txtComp = txtComp + comp[i].value + " ";
            }
        }return comp;
    ...
    function courseHistoryPos() {
        ...
        if (document.getElementById("positiveCoding").checked && genders == "Herr" ) {
            evaluation = "<div>" + genders + " " + inputValue +" war ein sehr lernbereiter, interessierter und ruhiger Kursteilnehmer. Er fand im Kurs stets ohne Aufforderung themenbezogene Tätigkeiten, sodass es nie zu einem Leerlauf kam. Herr Gomboc ist sehr motiviert, lernbereit und Interessiert an der IT, wodurch wir ihn als fähigen Kandidaten für die Folgeausbildung in der Programmierung in der Coders.Bay einschätzen.Generell erwarb Herr Gomboc Grundkenntnisse in der </div>";
